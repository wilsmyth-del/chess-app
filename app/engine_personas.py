...
def _load_modular_config(data):
    ...
    Creates personas from style_profiles only. Strength is resolved at runtime
    via assemble_persona_config() which merges strength + style dynamically.
    """
    ...
    # Strength (depth, elo, skill, mercy, endgame_depth) is merged at runtime.
...
def assemble_persona_config(style_name, strength_name=None, engine_skill=None):
    ...
    Args:
        style_name: Style profile key (e.g. "cautious", "reckless").
    ...
    Returns:
        dict with ALL fields needed by pick_move_with_multipv.
        Strength owns: depth, elo, skill, multipv, uci, mercy, endgame_depth_delta.
        Style owns: pick_temperature, curve, blunder_cap, endgame_temp_delta.
    """
    ...
    style = get_persona_config(key) or {}
